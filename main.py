...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:20032003a@127.0.0.1:5432/webarchive'
db.init_app(app)
...
def archive_page():
    try:
        ...
        domain_info_data = logic.get_domain_info(url)
...
def view_domain_info(encoded_url):
    try:
        decoded_url = unquote(encoded_url)
        print(f"Requested URL: {decoded_url}")
        domain_info_data = logic.get_domain_info(decoded_url)
